...
export const InspectionReportView: React.FC<InspectionReportViewProps> = ({ sample }) => {
  const [operatorName, setOperatorName] = useState('Dr. Elena Vance (Lead Metrology Engineer)');
  const [isEditingOperator, setIsEditingOperator] = useState(false);
  const [notes, setNotes] = useState(sample.notes || '');

  const reportData = backendApi.generateReport(sample, operatorName);

  const handleSaveNotes = (val: string) => {
    setNotes(val);
    backendApi.updateSample(sample.id, { notes: val });
  };
  ...
  const handleExportJSON = () => {
    ...
    link.download = `${sample.id}-metrology-certificate.json`;
    link.click();
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-xs gap-3">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
          >
      ...
      <div className="bg-white p-8 sm:p-12 rounded-3xl border border-slate-200 shadow-xl space-y-8 print:p-0 print:border-none print:shadow-none">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-6 rounded-2xl bg-slate-50 border border-slate-200">
          <div className="space-y-2 text-xs">
            ...
            {/* Editable Inspector Name */}
            <div className="flex items-center gap-2">
              <span className="text-slate-500 font-medium">Metrology Inspector:</span>
              {isEditingOperator ? (
                <input
                  type="text"
                  value={operatorName}
                  ...
                  autoFocus
                  className="rounded border border-blue-400 px-1.5 py-0.5 text-xs font-semibold text-slate-900 bg-white"
                />
              ) : (
                <span
                  onClick={() => setIsEditingOperator(true)}
                  className="font-semibold text-slate-900 cursor-pointer hover:text-blue-600 flex items-center gap-1"
                  title="Click to Edit Operator Name"
                >
                  {operatorName} <Edit3 className="h-3 w-3 text-slate-400" />
                </span>
              )}
            </div>
        ...
        {/* Custom Metrology Engineering Notes Field */}
        <div className="space-y-2 print:hidden">
          ...
          <textarea
            rows={3}
            placeholder="Add custom metrology analysis observations or packaging instructions..."
            value={notes}
            onChange={(e) => handleSaveNotes(e.target.value)}
            className="w-full rounded-2xl border border-slate-200 bg-slate-50/60 p-3 text-xs text-slate-900 focus:border-blue-500 focus:bg-white focus:outline-none"
          />
        </div>
