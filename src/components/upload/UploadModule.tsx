...
export const UploadModule: React.FC<UploadModuleProps> = ({
  ...
  const processFiles = (files: FileList | File[]) => {
    ...
    setUploading(true);
    setUploadProgress(0);
    setValidationInfo(`Validating ${fileArray.length} SEM Image Scan(s)...`);
    ...
    const interval = setInterval(() => {
      ...
      if (progress === 40) {
        ...
        setValidationInfo(`Constructing Wafer Lot Tensor Container for ${fileArray.length} scan(s)...`);
      } else if (progress >= 100) {
        clearInterval(interval);
        setUploading(false);

        let firstCreatedSample: WaferSample | null = null;

        fileArray.forEach((file, index) => {
          const customSample: WaferSample = {
            id: `waf-custom-${Date.now()}-${index}`,
            ...
            metrics: {
              psnr: Math.round((37.5 + Math.random() * 4.5) * 10) / 10,
              ssim: Math.round((0.978 + Math.random() * 0.018) * 1000) / 1000,
              noiseReductionPct: Math.round((93.5 + Math.random() * 4.5) * 10) / 10,
              restorationConfidence: 99.1,
              processingTimeMs: Math.round((14 + Math.random() * 8) * 10) / 10,
              gpuMemoryGb: 4.4,
              resolutionScale: '4x',
              inferenceSpeedFps: 148,
              snrGainDb: Math.round((13.5 + Math.random() * 3.5) * 10) / 10,
            },
            defects: [
              {
                id: `def-custom-${index + 1}`,
                label: 'Surface Contamination Particle',
                category: 'particle',
                bbox: { x: 35 + index * 5, y: 35 + index * 3, width: 14, height: 14 },
                confidence: 0.954,
                severity: 'major',
                areaUm2: 0.084,
          ...
          if (index === 0) firstCreatedSample = customSample;
