...
export function applyRestormerAI(
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      let sharpR = centerR + (centerR - avgR) * sharpeningFactor;
      let sharpG = copy[idx + 1] + (copy[idx + 1] - avgG) * sharpeningFactor;
      let sharpB = copy[idx + 2] + (copy[idx + 2] - avgB) * sharpeningFactor;

      // Contrast enhancement adjustment if specified
      if (contrastBoost !== 1.0) {
        sharpR = (sharpR - 128) * contrastBoost + 128;
        sharpG = (sharpG - 128) * contrastBoost + 128;
        sharpB = (sharpB - 128) * contrastBoost + 128;
      }
...
/**
 * Helper to download canvas content as PNG
 */
